...
- **Power:** ESP32 5V + LED strip 5V share common ground with ESP32 GND.
  INMP441 VDD from ESP32 3V3. **DRV8833 VCC currently runs from the
  ESP32's own 3.3V rail** — see section 5, this is a known constraint,
...
- **DRV8833 VCC is powered from the ESP32's own 3.3V logic rail.** This
  is acceptable for verified bench bring-up only and is explicitly **not**
  the intended production power architecture. Before any sustained or
  unattended operation, this needs a dedicated external motor supply with
  common ground, sized for actual current draw including startup inrush.
- **Motor engagement measurably disturbs LED output** while the two share
  this supply — physically observed, correlated with shared-supply power
  contention (no exact voltage sag has been instrumented). Motor movement
  is reliable with LEDs muted, inconsistent/weak while LEDs are active.
- Three different LED-coexistence strategies exist for this reason, and
  ...
  - **No mitigation at all** — `DanceEngine` and `MusicMotorController`
    ...
    the shared 3.3V rail simultaneously for as long as a session lasts —
