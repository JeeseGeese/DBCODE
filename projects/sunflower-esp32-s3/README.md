...
MAX98357A `VIN` is powered from the ESP32 `3.3V` pin; `GND` is the
same common ground. `SD` is presently a manually-controlled pin (GND =
amplifier shutdown/silent, 3.3V = amplifier enabled) — see the Speaker
...
- **LED power handling: none -- `DanceEngine` never touches LED mute
  ...
  share the ESP32's 3.3V rail (see [Power](#power) below) -- without any
...
shared 3.3V rail simultaneously for as long as the dance continues, not
...
same conservative-amplitude philosophy: the 8Ω/0.5W speaker is powered by
a 3.3V-supplied amplifier, so nothing here exceeds 20% of full scale.
